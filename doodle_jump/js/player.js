export class Player {
  constructor(game) {
    ...
    this.width = 295 * this.sizeModifier
    this.height = 358 * this.sizeModifier
    ...
    this.min_y = (this.game.platforms[0].height)
    this.min_vy = -9
    this.max_vy = this.game.platforms[0].height
    this.vy = this.min_vy
    this.weight = 0.2
    this.image = document.querySelector('#doodler')
    this.vx = 0
    this.max_vx = 4
  }
  ...
  update(inputHandler){
    ...
    if(this.collision()){
      ...
      new Audio('sound effects/fall.wav').play()
    }
  ...
  collision(){
    let result = false
    let playerHitBox = {x:this.x+5, y:this.y, width:this.width-28, height:this.height}
    this.game.enemies.forEach(enemy => {
